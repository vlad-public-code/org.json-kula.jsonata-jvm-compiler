...
final class WordNumbers {
     ...
     * Handles cardinals, ordinals ("first"→1), hyphenated composites ("twenty-one"→21),
     * and "and" connectors.
     */
    static String wordsToDigits(String input) {
        ...
        for (String word : words) {
            switch (word) {
                case "one",   "first"       -> { current += 1;   matched = true; }
                case "two",   "second"      -> { current += 2;   matched = true; }
                case "three", "third"       -> { current += 3;   matched = true; }
                case "four",  "fourth"      -> { current += 4;   matched = true; }
                case "five",  "fifth"       -> { current += 5;   matched = true; }
                case "six",   "sixth"       -> { current += 6;   matched = true; }
                case "seven", "seventh"     -> { current += 7;   matched = true; }
                case "eight", "eighth"      -> { current += 8;   matched = true; }
                case "nine",  "ninth"       -> { current += 9;   matched = true; }
                case "ten",       "tenth"       -> { current += 10;  matched = true; }
                case "eleven",    "eleventh"    -> { current += 11;  matched = true; }
                case "twelve",    "twelfth"     -> { current += 12;  matched = true; }
                case "thirteen",  "thirteenth"  -> { current += 13;  matched = true; }
                case "fourteen",  "fourteenth"  -> { current += 14;  matched = true; }
                case "fifteen",   "fifteenth"   -> { current += 15;  matched = true; }
                case "sixteen",   "sixteenth"   -> { current += 16;  matched = true; }
                case "seventeen", "seventeenth" -> { current += 17;  matched = true; }
                case "eighteen",  "eighteenth"  -> { current += 18;  matched = true; }
                case "nineteen",  "nineteenth"  -> { current += 19;  matched = true; }
                case "twenty",  "twentieth"  -> { current += 20;  matched = true; }
                case "thirty",  "thirtieth"  -> { current += 30;  matched = true; }
                case "forty",   "fortieth"   -> { current += 40;  matched = true; }
                case "fifty",   "fiftieth"   -> { current += 50;  matched = true; }
                case "sixty",   "sixtieth"   -> { current += 60;  matched = true; }
                case "seventy", "seventieth" -> { current += 70;  matched = true; }
                case "eighty",  "eightieth"  -> { current += 80;  matched = true; }
                case "ninety",  "ninetieth"  -> { current += 90;  matched = true; }
                case "hundred", "hundredth"  -> { current *= 100; matched = true; }
