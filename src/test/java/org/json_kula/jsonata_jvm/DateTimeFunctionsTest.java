...
class DateTimeFunctionsTest {
    ...
    void millis_returnsNumber() throws Exception {
        JsonNode result = FACTORY.compile("$millis()").evaluate(EMPTY);
    ...
    void millis_valueIsReasonable() throws Exception {
        ...
        long value = FACTORY.compile("$millis()").evaluate(EMPTY).longValue();
    ...
    void millis_frozenWithinSingleEvaluation() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$millis() = $millis()").evaluate(EMPTY);
    ...
    void now_returnsIso8601String() throws Exception {
        JsonNode result = FACTORY.compile("$now()").evaluate(EMPTY);
    ...
    void now_valueIsReasonable() throws Exception {
        long before = System.currentTimeMillis();
        String iso = FACTORY.compile("$now()").evaluate(EMPTY).textValue();
        long after = System.currentTimeMillis();
    ...
    void now_frozenWithinSingleEvaluation() throws Exception {
        JsonNode result = FACTORY.compile("$now() = $now()").evaluate(EMPTY);
    ...
    void now_andMillis_sameTimestamp() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$toMillis($now()) = $millis()").evaluate(EMPTY);
    ...
    void now_withPictureString() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$now(\"" + picture + "\")").evaluate(EMPTY);
    ...
    void now_withPictureAndTimezone() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$now(\"[H01]:[m01]\", \"+0530\")").evaluate(EMPTY);
        assertTrue(result.isTextual(), "Expected a string");
    ...
    void fromMillis_defaultIso() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$fromMillis(1510067557121)").evaluate(EMPTY);
    ...
    void fromMillis_missingReturnsNothing() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$fromMillis($notDefined)").evaluate(EMPTY);
    ...
    void fromMillis_roundTrip() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$toMillis($fromMillis(1510067557121))").evaluate(EMPTY);
    ...
    void toMillis_isoString() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$toMillis(\"2017-11-07T15:07:54.972Z\")").evaluate(EMPTY);
    ...
    void toMillis_isoStringNoMillis() throws Exception {
        ...
        JsonNode result = FACTORY.compile("$toMillis(\"2017-11-07T15:12:37Z\")").evaluate(EMPTY);
    ...
    void toMillis_roundTrip() throws Exception {
        ...
        JsonNode result = FACTORY.compile(
                "$fromMillis($toMillis(\"2017-11-07T15:12:37.121Z\"))").evaluate(EMPTY);
        assertEquals("2017-11-07T15:12:37.121Z", result.textValue());
    }

    @Test
    void toMillis_missingReturnsNull() throws Exception {
        JsonNode result = FACTORY.compile("$toMillis($notDefined)").evaluate(EMPTY);
        assertTrue(result.isNull(), "Expected null but got: " + result);
    }
}
