...
class OtherOperatorsTest {
    ...
    void conditional_non_boolean_coercion() throws Exception {
        ...
        assertEquals("yes", eval("1 ? \"yes\" : \"no\"").textValue());
        // Zero is falsy → false branch
        assertEquals("no", eval("0 ? \"yes\" : \"no\"").textValue());
    }

    @Test
    void conditional_missing_condition_is_falsy() throws Exception {
        // MISSING is falsy → false branch
        assertEquals("no", eval("$notDefined ? \"yes\" : \"no\"").textValue());
    }
    ...
    void elvis_null_is_falsy_returns_default() throws Exception {
        ...
        assertEquals("default", eval("null ?: \"default\"").textValue());
    }

    @Test
    void elvis_zero_is_falsy() throws Exception {
        assertEquals("default", eval("0 ?: \"default\"").textValue());
    }

    @Test
    void elvis_empty_string_is_falsy() throws Exception {
        assertEquals("default", eval("\"\" ?: \"default\"").textValue());
    }

    @Test
    void elvis_truthy_value_returned_directly() throws Exception {
        assertEquals("hello", eval("\"hello\" ?: \"default\"").textValue());
        assertEquals(42L, eval("42 ?: 0").longValue());
    }
    ...
    void binding_simple_value() throws Exception {
        assertEquals(5L, eval("($five := 5; $five)").longValue());
    }

    @Test
    void binding_arithmetic_expression() throws Exception {
        assertEquals(25L, eval("($x := 5; $x * $x)").longValue());
    }

    @Test
    void binding_lambda() throws Exception {
        assertEquals(9L, eval("($square := function($n) { $n * $n }; $square(3))").longValue());
    }

    @Test
    void binding_multiple_in_block() throws Exception {
        // Multiple bindings in one block, each separate
        assertEquals(15L, eval("($x := 5; $y := $x * 3; $y)").longValue());
    }
    ...
    void chain_two_functions() throws Exception {
        ...
        assertEquals("HELLO", eval("\"  hello  \" ~> $trim() ~> $uppercase()").textValue());
    }
    ...
    void chain_function_composition() throws Exception {
        ...
        assertEquals("HELLO WORLD",
                eval("($normalize := $uppercase ~> $trim; $normalize(\"   Hello    World   \"))").textValue());
    }
    ...
    void chain_with_partial_application() throws Exception {
        ...
        assertEquals("Winch",
                eval("($first5 := $substring(?, 0, 5); \"Winchester\" ~> $first5)").textValue());
    }
    ...
    void transform_does_not_mutate_source() throws Exception {
        ...
        JsonNode original = JsonNodeTestHelper.evaluate("Account.Order.Product[0].Price", ORDER_DOC);
        JsonNodeTestHelper.evaluate(expr, ORDER_DOC);
